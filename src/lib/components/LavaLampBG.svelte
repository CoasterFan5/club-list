<script lang="ts">
	...
	class Orb {
		...
		constructor() {
			...
			let color = 'hsla(355, 78%, light%, alp)'.replace( 'light', (56 + (Math.floor(Math.random() * 40) - 20)).toString());
...
<canvas hidden width={canvasWidth} height={canvasHeight} bind:this={fakeCanvas}></canvas>
<canvas bind:clientHeight={canvasHeight} bind:clientWidth={canvasWidth} width={canvasWidth} height={canvasHeight} bind:this={canvasElement} id="final">
